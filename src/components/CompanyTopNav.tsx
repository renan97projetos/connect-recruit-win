...
export function CompanyTopNav() {
  ...
  return (
    <header className="sticky top-0 z-50 h-16 bg-white border-b border-gray-200 flex items-center px-4 md:px-6 gap-4 md:gap-6">
      {/* Logo + nome empresa */}
      <Link to="/company" data-tour="company-logo" className="flex items-center gap-2.5 mr-2 min-w-0 flex-shrink-0">
        <Avatar className="h-11 w-11 rounded-md">
          {companyLogoUrl && (
            <AvatarImage key={companyLogoUrl} src={companyLogoUrl} alt={companyName} className="object-contain" />
          )}
          <AvatarFallback className="bg-primary text-primary-foreground text-sm rounded-md font-semibold">
            {companyInitials}
          </AvatarFallback>
        </Avatar>
